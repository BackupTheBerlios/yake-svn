...
# ifdef BOOST_WINDOWS
  std::string system_message( int sys_err_code )
  {
    std::string str;
    LPVOID lpMsgBuf;
	/* todo bei yake namespace :: */FormatMessageA( 
    ...
    /* todo bei yake namespace :: */LocalFree( lpMsgBuf ); // free the buffer
    while ( str.size()
      && (str[str.size()-1] == '\n' || str[str.size()-1] == '\r') )
...
#     endif
  ...
  std::string lookup_error_string( fs::error_code err_code )
  {
	  static std::string error_code_strings[] =
	  {
          "no_error",
          "system_error",     
          "other_error",      
	  ...
	  return error_code_strings[ static_cast< int >( err_code ) ];
  }

  std::string user_error_prep(
    const std::string & who,
		fs::error_code err_code )
  {
    return who + ": " + lookup_error_string( err_code );
  }
  ...
  std::string user_error_prep(
    const std::string & who,
		...
		fs::error_code err_code )
  {
    return who + ": \"" + path1.native_file_string() + "\": " + lookup_error_string( err_code );
  }
...
//  filesystem_error implementation  -----------------------------------------//
    ...
    filesystem_error::filesystem_error(
      const std::string & who,
      error_code err_code )
      : m_sys_err(0), m_err(err_code)
    {
      try
      {
        m_imp_ptr.reset( new m_imp );
        m_imp_ptr->m_who = who;
        m_imp_ptr->m_what = user_error_prep( who, err_code );
      }
      catch (...) { m_imp_ptr.reset(); }
    }
      ...
      : m_sys_err(0), m_err(err_code)
    {
      try
      {
        m_imp_ptr.reset( new m_imp );
        m_imp_ptr->m_who = who;
        m_imp_ptr->m_what = user_error_prep( who, path1, err_code );
        m_imp_ptr->m_path1 = path1;
      }
      catch (...) { m_imp_ptr.reset(); }
    }
